                ...
                .then(async res => {
                    ...
                    if (data.avatar_url) {
                        ...
                        if (parseInt(personId) === parseInt(authIdPerson)) {
                            // TOPO
                            const topo = document.querySelector('#kt_header_user_menu_toggle > div.symbol');
                            if (topo) {
                                topo.innerHTML = '';
                                const img = document.createElement('img');
                                img.className = 'symbol symbol-circle symbol-35px symbol-md-40px';
                                img.src = avatarUrl;
                                img.alt = 'user';
            ...
            btnRemove.addEventListener('click', function () {
                ...
                .then(data => {
                    if (data.status) {
                        ...
                        if (parseInt(personId) === parseInt(authIdPerson)) {
                            document.querySelectorAll('img[src*="avatar"]').forEach(img => img.remove());

                            const letra = '{{ strtoupper(substr(session('authNameFirst'), 0, 1)) }}';
                            const cor = '{{ match (session('authIdGender')) {
                                1 => 'primary',
                                2 => 'danger',
                                default => 'info',
                            } }}';

                            const alvoTopo = document.querySelector('#kt_header_user_menu_toggle > div.symbol');
                            if (alvoTopo) {
                                alvoTopo.innerHTML = '';
                                const span = document.createElement('span');
                                span.className = `symbol-label symbol-circle symbol-35px symbol-md-40px bg-light-${cor} text-${cor} fw-bold`;
                            ...
                            if (dropdown) {
                                ...
                                spanDropdown.className = `symbol-label bg-light-${cor} text-${cor} fw-bold`;
            ...
            btnCancel.addEventListener('click', function () {
                setTimeout(() => {
                    ...
                    .then(async res => {
                        ...
                        if (res.ok && data.avatar_url) {
                            wrapper.style.backgroundImage = `url(${data.avatar_url}?t=${new Date().getTime()})`;
                            if (btnRemove) btnRemove.style.display = 'inline-block';
                        } else {
                            wrapper.style.backgroundImage = 'url(/assets/media/svg/files/blank-image.svg)';
                            if (btnRemove) btnRemove.style.display = 'none';
